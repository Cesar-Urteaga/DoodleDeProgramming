...
void ofApp::update() {
	...
	ofColor color;
	bool flag = true;
	for (float x_start = 0; x_start < 720; x_start += 144)
	{
		flag = !flag;

		for (float x = x_start + 7; x < x_start + 137; x += span) {

			for (float y = 0; y < 720; y += span) {

				float noise_step = flag ? 0.045 : 0.01;
				float noise_value = ofMap(ofNoise(x * 0.01, y * 0.003 + ofGetFrameNum() * noise_step, ofGetFrameNum() * 0.01), 0, 1, 0, 10);
				if ((int)noise_value % 5 < 4) {
				...
				color.setHsb(ofMap(x_start, 18, 720, 0, 255), 230, 0);
				for (int i = 0; i < 4; i++) {
